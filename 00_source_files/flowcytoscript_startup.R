...
required.packages <- c(
  ...
  "parallel", "data.table", "remotes", "BiocManager",
  "coda", "emmeans", "EmbedSOM", "ConsensusClusterPlus", "flowCore",
  "flowWorkspace", "readxl", "scattermore", "Rphenograph", "parallelly"
)
...
bioconductor.packages <- c("ConsensusClusterPlus", "flowCore", "flowWorkspace")
...
If you're prompted to update, please do so.\n")
  ...
  if ( length(setdiff(github.packages, rownames(installed.packages()))) !=0 ){
    library(devtools)
