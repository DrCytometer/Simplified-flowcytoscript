...
if (input.file.type == 2){
  ...
  } else if( flowFrame@description$`$CYT` == "FACSDiscover S8" ){
...
## TBD: include transforms for Attune, Bigfoot
if ( input.file.type == 2 ){
  ...
  if( flowFrame@description$`$CYT` == "Aurora" ){
    ...
    log.decades <- 5
  } else if( flowFrame@description$`$CYT` == "EVEREST" ){
    width.basis <- -50
    max.value <- 262144
    log.decades <- 4.42
  } else {
...
if ( input.file.type == 2 ){
  ...
  if ( grepl("CYTOF|HELIOS", flowFrame@description$`$CYT` ) ){
    asinh.transform <- arcsinhTransform(a=1, b=1/5, c=0)
    dmrd.data <- apply( dmrd.data, 2, asinh.transform)
  } else {
    dmrd.data <- apply( dmrd.data, 2, biexp.transform )
    dmrd.data <- apply( dmrd.data, 2, FUN = "-", zero.point )
  }
